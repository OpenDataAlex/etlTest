...
class YamlParserTests(unittest.TestCase):
    ...
    def test_process_test_dir(self):
        ...
        expected_result = [{'DataMart\\UsersDim': {'suites': [{'suite': 'dataMart'}], 'processes': [{'tool': 'PDI',
                            'processes': [{'type': 'job', 'name': 'data_mart/user_dim_jb.kjb'}]}], 'tests': [{'query':
                                           {'from': 'users_dim', 'where': 'user_id = 2', 'result': 'sarah', 'select':
                                            'first_name', 'source': 'etlUnitTest'}, 'name': 'testFirstNameLower',
                                             'desc': 'Test for process that lower cases the first name field of a '
                                                     'users table record.'}, {'query': {'from': 'users_dim', 'where':
                                                     'user_id = 2', 'result': 'SARAH', 'select': 'first_name',
                                                     'source': 'etlUnitTest'}, 'name': 'testFirstNameUpper',
                                             'desc': 'Test for process that upper cases the first name field of a '
                                             'users table record.'}, {'query': {'from': 'users_dim', 'where':
                                             'user_id IN (1, 2)', 'result': [{1: '01-01-1900'}, {2: '02-02-2000'}],
                                             'select': 'birthday', 'source': 'etlUnitTest'},
                                             'name': 'testUserValidBirthday', 'desc': 'Test for valid birth '
                                             'dates.'}], 'dataset': [{'source': 'etlUnitTest', 'table': 'users',
                                                                      'records': [1, 2]}]}}]
